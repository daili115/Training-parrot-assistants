...
const EFFECT_ICONS: Record<VoiceEffect, any> = {
  normal: User,
  parrot: Bird,
  deep: AudioLines,
  robot: Zap,
...
const EFFECT_CONFIG = {
  normal:   { rate: 1.0, pitch: true },
  parrot:   { rate: 1.25, pitch: false },
  deep:     { rate: 0.85, pitch: false },
  robot:    { rate: 1.0, pitch: false },
  chipmunk: { rate: 1.5, pitch: false },
  baby:     { rate: 1.15, pitch: false },
  monster:  { rate: 0.7, pitch: false },
  alien:    { rate: 1.1, pitch: false },
  radio:    { rate: 1.05, pitch: false },
  ghost:    { rate: 0.75, pitch: true }
};
...
const PhraseCard: React.FC<PhraseCardProps> = ({ phrase, onDelete, onUpdateMastery, volume }) => {
  ...
  return (
    <div className="bg-white rounded-2xl md:rounded-3xl p-4 md:p-5 flex flex-col sm:flex-row items-center gap-4 md:gap-5 shadow-sm border border-slate-100 transition-all group relative overflow-hidden">
      <div 
      ...
      <div className="flex items-center gap-4 md:gap-5 w-full">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <h3 className="font-black text-slate-800 text-base md:text-lg truncate leading-none">{phrase.label}</h3>
            <div className="flex items-center gap-1 px-1.5 py-0.5 bg-slate-100 rounded-lg shrink-0">
              <Icon className="w-2.5 h-2.5 md:w-3 md:h-3 text-slate-500" />
              <span className="text-[7px] md:text-[8px] font-black text-slate-500 uppercase tracking-tighter">{phrase.effect}</span>
            </div>
          </div>
          <div className="flex items-center gap-3 text-[9px] md:text-[10px] text-slate-400 font-bold">
        ...
        <div className="flex flex-col items-end gap-1.5">
          <div className="flex items-center gap-1">
            ...
            <span className="text-[10px] md:text-xs font-black text-slate-600">{phrase.mastery}%</span>
          </div>
          <input 
