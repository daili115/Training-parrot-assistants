  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        ...
        <button 
          onClick={handleAdd}
          className="bg-slate-900 text-white px-4 py-2 rounded-xl font-bold hover:bg-slate-800 transition-all flex items-center gap-2"
        >
      ...
      <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
        {slots.length === 0 ? (
          <p className="text-center text-slate-400 py-4 text-sm italic">未设定训练计划</p>
        ) : (
          slots.map(slot => (
            <div 
              key={slot.id} 
              className={`flex items-center justify-between p-3 rounded-xl border transition-all ${
                slot.enabled ? 'bg-amber-50 border-amber-100' : 'bg-slate-50 border-slate-100 opacity-60'
              }`}
            >
              <div className="flex items-center gap-3">
                ...
                <span className={`font-bold ${slot.enabled ? 'text-amber-900' : 'text-slate-500'}`}>
                  {slot.time}
                </span>
              </div>
              <button 
