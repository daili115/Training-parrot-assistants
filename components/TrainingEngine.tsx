...
const TrainingEngine: React.FC<TrainingEngineProps> = ({ phrases, settings, onFinish, onPhrasePlayed }) => {
  ...
  useEffect(() => {
    ...
    const initNoiseMonitor = async () => {
      try {
        ...
        check();
      } catch (e) { console.debug("Noise monitor failed"); }
    };
    initNoiseMonitor();
  ...
  const playNext = async () => {
    ...
    } catch (err) { setStatus('waiting'); setTimeout(playNext, 2000); }
  };

  useEffect(() => {
    const timer = setInterval(() => {
      if (!isPaused) setTimeLeft(t => { if (t <= 1) { onFinish({ totalPlays: playCount, durationMinutes: settings.sessionDuration }); return 0; } return t - 1; });
    }, 1000);
